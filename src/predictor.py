...
class ScoringService(object):
    ...
    def get_features_for_predict2(cls, dfs, code, fin_columns, start_dt=TEST_START):
        ...
        feats["volatility_1month"] = (
            np.log1p(feats["EndOfDayQuote ExchangeOfficialClose"])
            .diff()
            .rolling(20)
            .std()
        )
    ...
    def predict(
        ...
        # print('df:')
        # print(df)
        # print(df.to_csv('df.csv'))

        df.budget = (df.budget / abs(df.label_low_20)).astype(int)
    ...
    def get_sentiment(cls, inputs, start_dt="2020-12-31"):
        # ニュース見出しデータへのパスを指定
        print('get_sentiment start_dt: ', start_dt)
